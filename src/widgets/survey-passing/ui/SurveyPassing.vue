...
import { type Survey, SurveyQuestionType, SurveyStatus } from '@/entities/Survey'
import { SurveyPassingCard, surveyApi } from '@/features/surveys'
...
const { isLoading: isSurveyLoading, data } = useQuery({
  ...
  queryFn: () => surveyApi.getSurveyById(route.params.id as string),
  retry: 1
})
...
const saveAnswer = async (object: EmitData) => {
  ...
  if (object.isLast) {
    surveyApi.finishingPassingSurvey(answer)
    router.push('/surveys')
  ...
  const answerResponse = await surveyApi.saveAnswerResponse(answer)
